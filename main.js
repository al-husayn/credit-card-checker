...
function idInvalidCardCompanies(invalidCCArr) {
  ...
  for (let i = 0; i < invalidCCArr.length; i++) {
    if (
      invalidCCArr[i][0] === 3 &&
      issuingCompaniesArr.indexOf("Amex (American Expres)") === -1
    ) {
      issuingCompaniesArr.push("Amex (American Expres)");
    } else if (
      invalidCCArr[i][0] === 4 &&
      issuingCompaniesArr.indexOf("Visa") === -1
    ) {
      issuingCompaniesArr.push("Visa");
    } else if (
      invalidCCArr[i][0] === 5 &&
      issuingCompaniesArr.indexOf("Mastercard") === -1
    ) {
      issuingCompaniesArr.push("Mastercard");
    } else if (
      invalidCCArr[i][0] === 6 &&
      issuingCompaniesArr.indexOf("Discover") === -1
    ) {
